...
export const CourseChat: React.FC<{ courseId: string }> = ({ courseId }) => {
  ...
  const load = async () => {
    const { data } = await supabase.from('messages').select('*, sender:sender_id(name, role, avatar_url)')
      .eq('course_id', courseId).order('created_at').limit(100);
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`course-${courseId}`)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages', filter: `course_id=eq.${courseId}` },
        async (payload) => {
          const { data } = await supabase.from('messages').select('*, sender:sender_id(name, role, avatar_url)')
  ...
  const send = async (e: React.FormEvent) => {
    ...
    setSending(true);
    await supabase.from('messages').insert({ course_id: courseId, sender_id: profile.id, content: text.trim() });
    setText('');
    setSending(false);
  };
                ...
                <motion.div key={msg.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  {!own && (
                    <div className="w-8 flex-shrink-0">
                      {showAvatar && <Avatar name={(msg as any).sender?.name} size="sm" />}
                    </div>
                  )}
                  <div className={`max-w-xs lg:max-w-md ${own ? 'items-end' : 'items-start'} flex flex-col gap-1`}>
                    {showAvatar && !own && (
                      <div className="flex items-center gap-2 px-1">
                        <span className="text-xs font-medium text-zinc-600 dark:text-zinc-400">{(msg as any).sender?.name}</span>
                        <Badge variant={(msg as any).sender?.role === 'faculty' ? 'info' : 'default'} size="sm">
                          {(msg as any).sender?.role}
                        </Badge>
                      </div>
                    )}
                    <div className={`px-4 py-2.5 rounded-2xl text-sm ${own
